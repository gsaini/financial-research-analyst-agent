...
with tab1:
    ...
    with pcols[0]:
        if st.button("RSI Oversold (30)", key="preset_rsi"):
            ...
            (
                st.success(f"Created RSI oversold alert for {sym}")
                if "error" not in r
    ...
    with pcols[1]:
        if st.button("Earnings 5 Days", key="preset_earnings"):
            ...
            (
                st.success(f"Created earnings alert for {sym}")
                if "error" not in r
                else st.error(r["error"])
            )
    with pcols[2]:
        if st.button("Golden Cross", key="preset_golden"):
            ...
            (
                st.success(f"Created golden cross alert for {sym}")
                if "error" not in r
                else st.error(r["error"])
            )
    with pcols[3]:
        if st.button("Volume 3x Spike", key="preset_vol"):
            ...
            (
                st.success(f"Created volume alert for {sym}")
                if "error" not in r
                else st.error(r["error"])
            )
    ...
    with ecol2:
        if st.button("Evaluate All Alerts", type="primary", key="eval_btn"):
            ...
            if triggered:
                ...
                for t in triggered:
                    st.info(
                        f"**{t.get('symbol', '')}**: {t.get('trigger_message', t.get('message', ''))}"
                    )
            else:
...
st.markdown("---")
st.caption(
    "Alerts are evaluated on-demand or via the WebSocket endpoint (/ws/alerts). In-memory storage — alerts reset on server restart."
)
